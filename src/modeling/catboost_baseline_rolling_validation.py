"""Fixed CatBoost versus Logistic baseline on 2020--2024 OOF folds."""
...
class CatBoostRollingValidationResult:
...
def run_catboost_baseline_rolling_validation(
    dataset: pd.DataFrame,
) -> CatBoostRollingValidationResult:
    """Fit fresh Logistic/CatBoost models per fold and score validation only."""
    ...
    fold_metrics = {}
    fold_counts = {}
    pooled_targets = {name: [] for name in ("logistic", "catboost")}
    pooled_probabilities = {name: [] for name in ("logistic", "catboost")}
    ...
    for validation_season in VALIDATION_SEASONS:
        ...
        catboost = CatBoostClassifier(
            loss_function="MultiClass", iterations=300, depth=4, learning_rate=0.03,
            l2_leaf_reg=5.0, random_seed=0, verbose=False,
            allow_writing_files=False, thread_count=1,
        )
        catboost.fit(train.loc[:, FEATURE_COLUMNS_FIXED], train["result"])
        if not np.array_equal(np.asarray(catboost.classes_), CLASS_ORDER):
            raise ValueError("Expected CatBoost class order [0, 1, 2].")
        catboost_probabilities = catboost.predict_proba(validation.loc[:, FEATURE_COLUMNS_FIXED])
        ...
        fold_metrics[validation_season]["catboost"] = _metrics(target, catboost_probabilities)
        for name, probabilities in (("logistic", logistic_probabilities), ("catboost", catboost_probabilities)):
    ...
    return CatBoostRollingValidationResult(fold_metrics, pooled_metrics, fold_counts)
