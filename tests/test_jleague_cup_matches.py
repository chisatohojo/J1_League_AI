import hashlib,json
import pytest
from src.collect.jleague_cup_matches import parse_sfms01_html,_get,_retain_j1_matches
...
def test_retains_j1_vs_unresolved_opponent_and_drops_non_j1_pair():
 import pandas as pd
 frame=pd.DataFrame({'home_is_j1':[True,False,False], 'away_is_j1':[False,True,False],
                     'home_team_id':['team_1',pd.NA,pd.NA], 'away_team_id':[pd.NA,'team_2',pd.NA]})
 out=_retain_j1_matches(frame, {'team_1','team_2'})
 assert len(out)==2
 assert out.iloc[0].home_is_j1 and out.iloc[1].away_is_j1

def test_retained_rows_have_a_season_j1_side():
 import pandas as pd
 frame=pd.DataFrame({'home_is_j1':[True,False], 'away_is_j1':[False,False]})
 out=_retain_j1_matches(frame, {'team_1'})
 assert (out.home_is_j1 | out.away_is_j1).all()
