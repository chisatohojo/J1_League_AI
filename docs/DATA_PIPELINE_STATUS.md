...
| Domestic Competitive Rest | 2015–2024 J1 **3,208** targets in rolling experiment; 2025/2026 inputs now materialized | Derived by `src/features/domestic_competitive_rest.py`; **no standalone feature CSV** | Team IDs and strictly prior domestic event date; nullable non-J1 Cup opponents | Frozen Model B feature group. AFC excluded, so AFC-club total competitive rest can be shorter. Historical feature module is scoped to 2015–2024; interim evaluator separately carries each completed target J1 match into later-date Rest history. |
| 2026 interim lockbox output | **70** saved prediction rows; Logistic training **3,588** ordinary J1 rows | `data/processed/modeling/2026_27_interim_lockbox_predictions.csv` | Target IDs unique; class order 0/1/2; same-date Elo/Rest buckets | Current chronology-corrected official interim report: A **40/70**, Log Loss **0.996370**, Brier **0.592515**; B **38/70**, Log Loss **1.001462**, Brier **0.596178**. Not a fresh selection set; see [2026_27_INTERIM_LOCKBOX_EVALUATION.md](2026_27_INTERIM_LOCKBOX_EVALUATION.md). |
...
| First 2026/27 lockbox run | 2026 Cup/Emperor inputs absent; Brier was averaged over matches **and classes** | **Invalid**. Materialized 4+19 Cup rows and corrected Brier to `mean(sum(..., axis=1))`. The subsequent 36/70 B run was **superseded** after correcting omitted prior-target Rest history; current result is 38/70. |
...
| Prior-date Domestic Rest and 90-minute Hyakunen Elo update | Current target and same-date peer results do not enter pre-match features; the target J1 event enters Rest history for later dates. Same-date target Elo reads all features before applying that date's results. |
| Lockbox input preflight and Brier correction | Current preflight hard-fails 2025 J1/Cup/Emperor **380/56/41**, 2026 Cup/Emperor **4/19**, Hyakunen **200**, target **70**, and schedule **380** (310 future); Brier uniform invariant is 2/3. |
