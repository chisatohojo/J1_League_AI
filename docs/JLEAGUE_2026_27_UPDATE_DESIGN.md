...
確認日: 2026-09-15（日本時間）。**設計確認後、更新アダプターと保存原本による初回bootstrapを実装・検証済み。未コミット。**
...
この規則で確認したのは34583（2026-09-13、東京Ｖ－千葉、1-1、result=1）の1件。
残り69件に同じ状態を推測で付与しない。
...
候補69件は終了していないという意味ではなく、今回保存した試合別終了証拠がないという意味。
次の運用段階では確認対象を選び、公式matchページの根拠を必要分だけ保存して新snapshotへ加える。
