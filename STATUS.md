...
2021年J1の20クラブ制・大会構造検証 — 完了、未コミット。2018～2020年・2022年以降と一括取得は未着手。
...
今回は2021年の大会構造検証・正規化・回帰テスト・調査報告まで完了。2021年対応のコード・テスト・文書は未コミット。
2015～2017年の調査・対応と解析共通化はユーザーがcommit・push済み。
2021年作業の開始時はクリーン。2026-09-14の再開時は変更5ファイル・未追跡3ファイルがあり、すべて保持して再開した。
再開後は実装・テストを変更せず、キャッシュで最終検証し、STATUS・CHANGELOG・関連文書へ結果を記録した。
今回Git add / commit / pushは行っていない。報告時点で停止する。
新規取得は2021年のみ。2018～2020年・2022年以降や一括取得器には進んでいない。既存CSV Validationは変更していない。
...
- `HEAD` / `origin/main`: `3116b12`（2017年対応）。解析共通化は `9c165d0`、2016年調査は `43efdcf`、2015年調査は `79ade3b`
- `HEAD...origin/main`: ローカルのみ0件 / リモートのみ0件。同期は不要
- 変更7ファイル: 共通モジュール・共通CLI・README・STATUS・CHANGELOG・DATA_SOURCES・DECISIONS
- 未追跡3ファイル: 2021年調査報告・専用テスト・2017年出力固定ハッシュ。すべて未ステージ
...
2. `docs/JLEAGUE_2021_RESEARCH.md` と人間レビュー、今回の未コミット変更・回帰テストを確認する。
3. 別年度へ進む依頼を受けたら、対象年の大会方式・stage・期待試合数を少量調査する。
   今回確認した4年度の形式を未調査年度に仮定しない。2022年以降へは今回進まない。
...
今回の完全一致検証には2021年対応前に退避した2015～2017年の8実出力を使用した。
再開後の再生成は一時ディレクトリで実施し、退避結果・既存成果物とのバイト一致を確認した。
pytestは人工データと変更前コードによる固定ハッシュで検証し、実データと通信を必要としない。

## 共通解析の構成と互換性

- 共通処理: `src/collect/jleague.py`。HTML解析・キャッシュ読取・集計は明示的な年度引数を受け取る。
- 共通CLI: `scripts/inspect_jleague.py --year 2015` / `--year 2016` / `--year 2017` / `--year 2021`。
...
- 2015年はCSV・集計JSON、2016年はCSV・集計JSON・人間レビューを従来と同じ名前・内容で出力。
  2017年・2021年もCSV・集計JSON・人間レビューを出力する。
- 2016年は2015年、2017年は2016年、2021年は2017年キャッシュと比較する。2020年を要求せず、自動巡回・取得機能は持たない。
- `SEASON_FORMATS` に公式確認済みのクラブ数とstage内対戦回数を設定し、`SEASON_STAGES` の節数も導出する。
  2015/2016年は18クラブ・1st/2nd各1回、2017年は18クラブ・full_season2回、2021年は20クラブ・full_season2回。
...
- `tests/test_jleague_2021.py` に2021年向け33件を追加し、2017年の `3116b12` 由来固定ハッシュも検証する。
  独立レビューでも旧コードを実行し、3出力のLF/CRLF双方のハッシュ一致を確認した。
- 2021年の初回取得でData SiteへのGETは1回。再開後は追加アクセス0回。
  2015～2017年のHTML・metadata6ファイルと8出力はバイト列も不変。
...
- Data Siteの2018～2020年・2022年以降は未取得。確認した4年度以外の大会形式・一括取得・訂正データの更新運用は未実装。
...
- [ ] チーム・スタジアム名称マスター
- [ ] 2018～2020年・2022年以降の調査・取得（明示的な依頼後の別作業）
