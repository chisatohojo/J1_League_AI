...
2018～2020年J1の取得・共通検証 — 完了、未コミット。2015～2021年を検証済み。2022年以降は未着手。
...
今回は2018～2020年の取得・共通処理適用・Validation・回帰テスト・調査報告まで完了、未コミット。
2015～2017年・2021年の調査・対応と解析共通化はcommit・push済み。今回の開始時はクリーンだった。
...
新規取得は2018～2020年のみ。2022年以降と自動取得アダプターには進んでいない。既存CSV Validationは変更していない。
...
- `HEAD` / `origin/main`: `ec065e7`（2021年対応）。2017年対応は `3116b12`、解析共通化は `9c165d0`
- `HEAD...origin/main`: ローカルのみ0件 / リモートのみ0件。同期は不要
- 変更9ファイル: 共通モジュール・共通CLI・既存テスト3ファイル・README・STATUS・CHANGELOG・DATA_SOURCES
- 未追跡1ファイル: `docs/JLEAGUE_2018_2020_RESEARCH.md`。すべて未ステージ
...
2. `docs/JLEAGUE_2018_2020_RESEARCH.md` と各年の人間レビュー、今回の未コミット変更・回帰テストを確認する。
3. 別年度へ進む依頼を受けたら、対象年の大会方式・stage・期待試合数を少量調査する。
   今回までに確認した7年度の形式を未調査年度に仮定しない。2022年以降へは今回進まない。
...
今回の完全一致検証には作業開始時に退避した2015/2016/2017/2021年の11実出力を使用した。
...
- 共通CLI: `scripts/inspect_jleague.py --year <年>`。2015～2021年を指定できる。
...
- 2015年はCSV・集計JSON、2016年はCSV・集計JSON・人間レビューを従来と同じ名前・内容で出力。
  2017～2021年もCSV・集計JSON・人間レビューを出力する。
- 2016～2020年は前年キャッシュと比較する。2021年の比較先は既存出力保護のため2017年を維持。
  自動巡回・ネットワーク取得機能は持たない。
- `SEASON_FORMATS` に公式確認済みのクラブ数とstage内対戦回数を設定し、`SEASON_STAGES` の節数も導出する。
  2015/2016年は18クラブ・1st/2nd各1回、2017～2020年は18クラブ・full_season2回、2021年は20クラブ・full_season2回。
...
- `tests/test_jleague_2021.py` の正常系3テストを2018～2021年の共通parameterized testに拡張し、9ケース追加。
  2021年の異常系と2017年の `3116b12` 由来固定ハッシュ検証は維持する。
  独立レビューでも旧コードを実行し、3出力のLF/CRLF双方のハッシュ一致を確認した。
- 今回のData SiteへのGETは2018～2020年各1回、計3回。解析・再生成では追加アクセス0回。
  2015/2016/2017/2021年のHTML・metadata8ファイルと11出力はバイト列も不変。
...
- Data Siteの2022年以降は未取得。確認した7年度以外の大会形式・自動取得アダプター・訂正データの更新運用は未実装。
